...
def bili_model_with_classifier(image_shape=IMG_SIZE, embedding_dim=128, num_classes=3):
    """
    Builds a MobileNetV2-based model that returns a classifier (softmax) output.
    The MobileNetV2 base is given an explicit name by setting base_model._name
    after construction because tf.keras.applications.MobileNetV2 does not accept
    a 'name' keyword.
    """
   ...
   # Load a MobileNetV2 base; replace weights path or set weights='imagenet' to use TF Hub weights
    ...
    # set the internal name so model.get_layer(name) can find it later
    base_model._name = "mobilenet_base"

    # Start with the base frozen; we'll unfreeze selectively when fine-tuning.
    ...
    # Model returns the classifier output (we'll also extract embedding later)
...
def main():
    ...
    # unfreeze_last_n = 5  # try 20..50 depending on backbone depth and dataset size
